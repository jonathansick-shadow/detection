...
typedef std::vector<Defect::PtrT>::const_iterator Defect_citerT;


/************************************************************************************************************/
/*
 * Classify an vector of Defect::PtrT for the given row, returning a vector of 1-D
 * Defects (i.e. y0 == y1 - 1).  In general we can merge in saturated pixels at
 * this step, although we don't currently do so.
 */
static std::vector<Defect::PtrT>
classify_defects(std::vector<Defect::PtrT> const & badList, // list of bad things
                 int const y,           // the row to process
                 int const ncol,        // number of columns in image
                 int = 0                // number of rows in image
                ) {

    std::vector<Defect::PtrT> badList1D;
    
    for (Defect_citerT begin = badList.begin(), end = badList.end(), bri = begin; bri != end; ++bri) {
        Defect::PtrT defect = *bri;
        ...
        for (++bri; bri != end; ++bri) {
            ...
            if (y < defect->getY0() || y >= defect->getY1() || x1 < defect->getX0()) {
                --bri;
                break;
            }
            x1 = defect->getX1();
        ...
        int const nbad = x1 - x0;
        assert(nbad >= 1);

        defect = Defect::PtrT(new Defect(vw::BBox2i(x0, y, nbad, 1)));
    ...
    for (Defect_citerT begin = badList1D.begin(), end = badList1D.end(), bri = begin; bri != end; ++bri) {
        ...
        int const nbad = defect->getX1() - defect->getX0();
        assert(nbad >= 1);
 ...
 */
        if(bri != begin) {
            ...
            if(defect->getPos() == Defect::RIGHT || defect->getPos() == Defect::WIDE_RIGHT) {
                ...
                Defect::PtrT const defect_m = *(bri - 1);
                assert(defect_m->getX1() <= defect->getX0());

                if(defect_m->getX1() == defect->getX0() - 1) {
        ...
        if(bri + 1 != end) {
            if(defect->getPos() == Defect::LEFT || defect->getPos() == Defect::WIDE_LEFT) {
                ...
                Defect::PtrT const defect_p = *(bri + 1);

                if(defect->getX1() == defect_p->getX0() - 1) {
...
static void do_defects(std::vector<Defect::PtrT> const & badList, // list of bad things
                       ...
                       typename lsst::afw::image::MaskedImage<ImageT, MaskT> & mi, // data to fix
    ...
    MIAccessorT ptr(mi);
    ptr.advance(0, y);
    ImageT *out = &(*ptr.image);
    MaskT *mask = &(*ptr.mask);

    for (Defect_citerT begin = badList.begin(), end = badList.end(), bri = begin; bri != end; ++bri) {
        Defect::PtrT const defect = *bri;
       
        if (y < defect->getY0() || y >= defect->getY1()) {
            continue;
        }

        int const bad_x1 = defect->getX0();
        int const bad_x2 = defect->getX1() - 1;

        for (int c = bad_x1; c <= bad_x2; ++c) {
            mask[c] |= interpBit;
        }

        switch (defect->getPos()) {
          case Defect::LEFT:
            assert(bad_x1 >= 0 && bad_x2 + 2 < ncol);
            ...
            switch (defect->getType()) {
              ...
              case 030:             /* ...##, <noise^2> = 0 */
                ...
                val = 1.0933*out2_1 - 0.0933*out2_2;
                out[bad_x2 - 1] = (val < min) ? out2_1 : val;
              ...
              case 0140:            /* .....##, <noise^2> = 0 */
                ...
                val = 0.6968*out2_1 + 0.3032*out2_2;
                out[bad_x2 - 2] = (val < min) ? out2_1 : val;
              ...
              case 0600:            /* .......##, <noise^2> = 0 */
                ...
                val = 0.5370*out2_1 + 0.4630*out2_2;
                out[bad_x2 - 3] = (val < min) ? out2_1 : val;
              ...
              case 03000:           /* .........##, <noise^2> = 0 */
                ...
                val = 0.5041*out2_1 + 0.4959*out2_2;
                out[bad_x2 - 4] = (val < min) ? out2_1 : val;
          ...
          case Defect::WIDE_LEFT:
            ...
            switch (defect->getType()) {
              ...
              case 03:            /* ?##, <noise^2> = 0 */
                ...
                for(int j = bad_x1;j < bad_x2 - 5;j++) {
                    out[j] = val;
                }

                val = 0.5003*out2_1 + 0.4997*out2_2;
                out[bad_x2 - 5] = (val < min) ? out2_1 : val;
          ...
          case Defect::RIGHT:
            ...
            switch (defect->getType()) {
              ...
              case 030:             /* ##..., <noise^2> = 0 */
                ...
                val = -0.0933*out1_2 + 1.0933*out1_1;
                out[bad_x2 - 1] = (val < min) ? out1_1 : val;
              ...
              case 0140:            /* ##....., <noise^2> = 0 */
                ...
                val = 0.3032*out1_2 + 0.6968*out1_1;
                out[bad_x2 - 2] = (val < min) ? out1_1 : val;
              ...
              case 0600:            /* ##......., <noise^2> = 0 */
                ...
                val = 0.4630*out1_2 + 0.5370*out1_1;
                out[bad_x2 - 3] = (val < min) ? out1_1 : val;
              ...
              case 03000:           /* ##........., <noise^2> = 0 */
                ...
                val = 0.4959*out1_2 + 0.5041*out1_1;
                out[bad_x2 - 4] = (val < min) ? out1_1 : val;
          ...
          case Defect::WIDE_RIGHT:
            ...
            switch (defect->getType()) {
              case 03:			/* ##?, S/N = infty */
                ...
                val = 0.4997*out1_2 + 0.5003*out1_1;
                out[bad_x1 + 5] = (val < min) ? out1_1 : val;

                val = 0.5000*out1_2 + 0.5000*out1_1;
                val = (val < min) ? out1_1 : val;

                for(int j = bad_x1 + 6;j <= bad_x2;j++) {
                    out[j] = val;
                }
                break;
              default:
          ...
          case Defect::NEAR_RIGHT:
            if(defect->getPos() == Defect::MIDDLE) {
                assert(bad_x1 >= 2 && bad_x2 + 2 < ncol);
                out1_2 = out[bad_x1 - 2];
                out2_2 = out[bad_x2 + 2];
            } else if(defect->getPos() == Defect::NEAR_LEFT) {
                assert(bad_x1 >= 1 && bad_x2 + 2 < ncol);
                out1_2 = -1;		/* NOTUSED */
                out2_2 = out[bad_x2 + 2];
            } else if(defect->getPos() == Defect::NEAR_RIGHT) {
                assert(bad_x1 >= 2 && bad_x2 + 1 < ncol);
                out1_2 = out[bad_x1 - 2];
                out2_2 = -1;		/* NOTUSED */
            } else {
            ...
            switch (defect->getType()) {
              ...
              case 042:			/* #...#., <noise^2> = 0, sigma = 1 */
                ...
                val = 0.5000*out1_1 + 0.5000*out2_1;
                out[bad_x2 - 1] = (val < min) ? 0.5*(out1_1 + out2_1):  val;
              ...
              case 0202:            /* #.....#., <noise^2> = 0, sigma = 1 */
                ...
                val = 0.5000*out1_1 + 0.5000*out2_1;
                out[bad_x2 - 2] = (val < min) ? 0.5*(out1_1 + out2_1):  val;
              ...
              case 01002:		/* #.......#., <noise^2> = 0, sigma = 1 */
                ...
                val = 0.5000*out1_1 + 0.5000*out2_1;
                out[bad_x2 - 3] = (val < min) ? 0.5*(out1_1 + out2_1):  val;
              ...
              case 04002:           /* #.........#., <noise^2> = 0 */
                ...
                val = 0.5000*out1_1 + 0.5000*out2_1;
                out[bad_x2 - 4] = (val < min) ? 0.5*(out1_1 + out2_1) : val;
              ...
              case 020003:          /* #...........##, <noise^2> = 0 */
                ...
                out[bad_x2 - 5] = (val < min) ? 0.5*(out1_1 + out2_1) : val;
          ...
          case Defect::WIDE_NEAR_RIGHT:
            ...
            switch (defect->getType()) {
              ...
              case 06:			/* #?#., <noise^2> = 0 */
                ...
                for(int j = bad_x1 + 6;j < bad_x2 - 5;j++) {
                    out[j] = val;
                }
              ...
              case 07:           /* #?##, <noise^2> = 0 */
                ...
                val = 0.4707*out[bad_x1 - 1] + 0.2646*out[bad_x2 + 1] + 0.2646*out[bad_x2 + 2];
...
struct Sort_DefectPtr {
    bool operator() (Defect::PtrT const a, Defect::PtrT const b) const {
        return a->getX0() < b->getX0();
    }
};

template<typename ImageT, typename MaskT>
void lsst::detection::interpolateOverDefects(MaskedImage<ImageT, MaskT> &image, ///< Image to patch
 ...
 */
    int const ncol = static_cast<int>(image.getCols()); // cast away "unsigned"
    ...
    for (int r = 0; r != nrow; r++) {
        std::vector<lsst::detection::Defect::PtrT> badList1D = classify_defects(badList, r, ncol);
        do_defects(badList1D, r, image, interpBit, std::numeric_limits<ImageT>::min());
    }
}
 ...
 */
template <typename ImageT, typename MaskT>
ImageT interp::singlePixel(int x, ///< column coordinate of the pixel in question
                                     ...
                                     ImageT minval ///< minimum acceptable value
                                    ) {
#if SDSS
   ...
   return std::numeric_limits<ImageT>::min();   
}
...
template
void lsst::detection::interpolateOverDefects(lsst::afw::image::MaskedImage<imagePixelType, maskPixelType> &image,
                                             PSF const &psf,
                                             std::vector<lsst::detection::Defect::PtrT> &badList
                                            );
template
...
#if 1
template
void lsst::detection::interpolateOverDefects(lsst::afw::image::MaskedImage<double, maskPixelType> &image,
                                             PSF const &psf,
                                             std::vector<lsst::detection::Defect::PtrT> &badList
                                            );
template
